...
class CarlaSimulator(DrivingSimulator):
	...
	def __init__(self, carla_map, map_path, address='127.0.0.1', port=2000, timeout=10.0,
...
class CarlaSimulation(DrivingSimulation):
	def __init__(self, scene, client, tm, timestep, render, record, scenario_number, verbosity=0):
		...
		if 'z' in scene.params:
			self.z = scene.params.get("z")
		else:
			self.z = None
	...
	def createObjectInSimulator(self, obj):
		...
		if self.z is not None:
			loc = utils.scenicToCarlaLocation(obj.position, z = self.z, world=self.world, blueprint=obj.blueprint)
		else:
		...
		if carlaActor is None:
			self.destroy()
			raise SimulationCreationError(f'Unable to spawn object {obj.blueprint}')
