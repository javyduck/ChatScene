...
class RouteReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        if len(prepared_collision_rewards)-1 < batch_size//4:
            ...
            sample_index = np.random.choice(np.arange(1, len(prepared_rewards)), size=batch_size - batch_size//4, replace=False)
            collision_sample_index = np.random.choice(np.arange(1, len(prepared_collision_rewards)), size=batch_size//4, replace=False)
